...
import { IAPIResponse, IMovieDetail, getMovie, getNowPlaying, makeBgPath, makeImagePath } from "../api";
...
export default function NowPlaying() {
    ...
    const {data: detailData} = useQuery<IMovieDetail>({
        queryKey: ["detailNow", movieId],
        queryFn: () => getMovie(String(movieId)),
    })
    const onCardClicked = (id: number) => {
        navigate(`/movie/${id}`)
    }
    const onCloseModal = () => navigate("/now-playing");
                            ...
                            {moviePathMatch ? (
                                <>
                                    <Overlay 
                                        ...
                                        animate={{opacity: 1}}    
                                    />
                                    <MovieModal layoutId={moviePathMatch.params.id + "n"} style={{backgroundColor: "rgba(0,0,0,1"}}>
                                        {clickedMovie && (
                                            <>
